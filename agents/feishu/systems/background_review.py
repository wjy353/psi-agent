...
_SKILL_REVIEW_PROMPT = (
    ...
    "Preference order (MUST list before any create; same as skill-authoring-when):\n"
    "  0. skill_manage(action='list') first — never create blind.\n"
    "  1. PATCH a currently-loaded skill if it covers the new learning.\n"
    "  2. PATCH an existing similar/umbrella skill (list + view; same domain "
    "    = patch, including agent_editable bases like feishu-resume-review).\n"
    "  3. CREATE a new class-level umbrella skill ONLY when nothing similar exists.\n"
    "     Name at the class level - NOT a PR number, error string, or "
    "'fix-X / debug-Y' session artifact. Do NOT spawn parallel skills "
    "for one domain.\n\n"
